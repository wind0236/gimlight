...
import Gimlight.UI.Draw.Config
  ( logRows,
    tileColumns,
    tileHeight,
    tileRows,
  ...
  where
    statusAndMapGrid =
      hstack
        [ mapGrid eh,
          statusGrid eh c
            `styleBasic` [width $ fromIntegral $ windowWidth - tileWidth * tileColumns]
        ]
...
mapGrid eh =
  zstack (mapWidget eh : (mapItems eh ++ mapActors eh))
  ...
  where
    ...
    cell c =
      zstack (catMaybes [lowerLayerAt c, upperLayerAt c, Just $ shadowAt c])
        `styleBasic` [width $ fromIntegral tileWidth, height $ fromIntegral tileHeight]
    ...
    isItemDrawed position =
      let displayPosition = itemPositionOnDisplay position
       ...
       in V2 0 0 <= displayPosition
            && displayPosition < V2 tileColumns tileRows
            && isVisible
    d = getCurrentDungeon eh
